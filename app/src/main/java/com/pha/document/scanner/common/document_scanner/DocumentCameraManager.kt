...
import com.pha.document.scanner.common.document_scanner.analyzer.DocumentAnalyzer
...
class DocumentCameraManager(
    ...
    private val liveDetector: LiveDocumentDetector,
...
)
{
    ...
    {
        return DocumentAnalyzer(buildLiveDetector())
    }
    
    private fun buildLiveDetector(): LiveDocumentDetector
    {
        val listener = object : LiveDocumentDetector.Listener
        {
            ...
            override fun onTaskFailed(task: DocumentDetectionTask, code: Int)
            {
                when (code)
                {
                    LiveDocumentDetector.ERROR_MULTI_DOC ->
                    {
                        guidelineView.text = "Please make sure there is only one document on the screen."
                    }
                    
                    LiveDocumentDetector.ERROR_NO_DOC ->
                    {
                        guidelineView.text = "Please make sure there is a document on the screen."
                    }
                    
                    LiveDocumentDetector.ERROR_OUT_OF_DETECTION_RECT ->
                    {
                        guidelineView.text = "Please make sure there is a document in the Rectangle."
                    }
